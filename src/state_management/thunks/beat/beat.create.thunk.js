...
const beatCreateThunk = createAsyncThunk(
  ...
  async (data, {fulfillWithValue, rejectWithValue, getState}) => {
    ...
    try {
      const snapshot = await Beat.create(data.name, data.genre, data.bpm, data.general_price, data.exclusive_price, data.path, data?.category, data.coverUrl, data.beatUrl)
